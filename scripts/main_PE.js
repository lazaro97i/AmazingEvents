...
let cardsPastE = amzEvents.filter( eventData => new Date(eventData.date).getFullYear() < 2022);
...
searchContainer.addEventListener("input", () =>{
    let filterCardsBySearch = filterAll(cardsPastE,search);
...
checkContainer.addEventListener("change", ()=>{
    let filterCardsByCategory = filterAll(cardsPastE,search);
